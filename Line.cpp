...
void Line::rotateShape(double degree)
{
	Coordinates center{};
	center.setX((beginPoint.getX() + endPoint.getX()) / 2);
	center.setY((beginPoint.getY() + endPoint.getY()) / 2);

	double radius = distanceBetweenPoints(beginPoint, center);
	
	beginPoint.setX(radius * cos(angleToPoint(center, getBegin()) + degree));
	beginPoint.setY(radius * sin(angleToPoint(center, getBegin()) + degree));

	endPoint.setX(radius * cos(angleToPoint(center, getEnd()) + degree));
	endPoint.setY(radius * sin(angleToPoint(center, getEnd()) + degree));
}
